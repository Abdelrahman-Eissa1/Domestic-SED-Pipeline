...
*   **Observation:** Continuous sounds (e.g., `running_water`) significantly outperformed transient events, as short sounds are often diluted by silence within the 1-second baseline window.
...
*   **Final Score:** The refined system achieved a balanced Macro $F_1$ of **0.4668**.

## 📊 Technical Visualizations

| Technical Stage | Primary Visualization | Key Insight |
| :--- | :--- | :--- |
| **Baseline Benchmark** | ![Baseline](visuals/baseline_f1_scores.png) | High consistency across splits; steady-state signals have a clear detection advantage. |
| **Hyperparameter Tuning** | ![Table](visuals/rf_hyperparameter_table.png) | 50 trees provided the optimal balance between variance reduction and compute. |
| **Post-Processing Delta** | ![Delta](visuals/f1_tradeoff_barchart.png) | Median filtering improves stability for continuous classes but penalizes transient accuracy. |
| **Qualitative Audit (1)** | ![Spec1](visuals/rf_qualitative_analysis.png) | Analysis of 1-second dropouts (flickering) in the `keychain` and `footsteps` classes. |
| **Qualitative Audit (2)** | ![Spec2](visuals/rf_qualitative_analysis_2.png) | Identified class confusion between spectrally similar hums (Microwave vs. Coffee Machine). |
